...
function Localization:t(key, ...)
    ...
    if not translation or translation == "" then
        ...
        local fallbacks = {
            ...
            menu_receive_from_pc = "从PC接收",
            receiver_stopped = "已停止接收",
            receiver_waiting_fmt = "等待PC连接，端口 %d。\n\n设备IP：%s\n\n在PC上运行：\n  python generator.py",
            receiver_start_failed = "无法启动接收器：%s",
            receiver_data_received = "已从PC接收 X-Ray 数据。\n正在重新加载…",
            receiver_data_loaded = "X-Ray 数据已加载。",
            receiver_write_failed = "接收失败：%s",
            error_no_book_open = "错误：当前未打开书籍",
            ai_preparing = "AI 正在准备分析...",
            ai_analysis_cancelled = "AI 分析已取消",
            btn_stop_bilingual = "停止 (Stop)",
            model_config_changed = "上次使用的模型配置已变更，请重新选择",
