...
-- Providers mirror generator.py: OpenAI, Claude, Groq, Gemini, DeepSeek.
-- All except Gemini (native handler) and Claude (Anthropic Messages API) are
-- OpenAI-compatible and share _callOpenAICompatibleRaw via their endpoint.
AIHelper.providers = {
    openai = {
        name = "OpenAI",
        type = "openai",
    ...
    -- Retained for backward compatibility with existing installs/config.
    chatgpt = {
        name = "ChatGPT",
        type = "chatgpt",
        enabled = true,
...
-- Providers exposed in the UI, ordered like generator.py's CLI menu.
AIHelper.provider_order = { "openai", "claude", "groq", "gemini", "deepseek" }

-- OpenAI-compatible /models listing endpoints per provider (mirrors ai_client.py).
AIHelper.provider_models_endpoint = {
...
-- In-memory cache of fetched model lists (per provider).
AIHelper._models_cache = {}

-- Substrings that never make sense for text analysis, across all providers
-- (image/audio/embedding/safety models, e.g. Gemini's "nano-banana" image model).
local MODEL_BLOCKLIST = {
    "embedding", "embed", "whisper", "tts", "text-to-speech", "speech",
    "audio", "realtime", "transcribe", "moderation", "rerank", "reranker",
    "image", "vision", "dall-e", "dalle", "imagen", "veo", "sora",
    "nano-banana", "banana", "guard", "orpheus", "ocr", "computer-use",
}

local function _modelBlocked(id)
    local low = id:lower()
    for _, bad in ipairs(MODEL_BLOCKLIST) do
        if low:find(bad, 1, true) then return true end
    end
    return false
end

-- Per-provider keep predicate applied after the shared blocklist.
local MODEL_KEEP = {
    -- Gemini: keep only flash / flash-lite (cheap, capable); drop pro/experimental.
    gemini = function(id) return id:lower():find("flash", 1, true) ~= nil end,
    -- OpenAI: keep GPT / o-series chat models only.
    openai = function(id)
        local low = id:lower()
        return low:find("gpt", 1, true) ~= nil
            or low:match("^o%d") ~= nil
            or low:find("chatgpt", 1, true) ~= nil
    end,
    -- claude / groq / deepseek: keep everything not blocked.
}

-- Sort Gemini flash models cheapest-first (flash-lite before flash, newer first).
local function _sortGeminiFlash(models)
    local lite, flash = {}, {}
    for _, m in ipairs(models) do
        if m:lower():find("flash-lite", 1, true) or m:lower():find("flash_lite", 1, true) then
            table.insert(lite, m)
        elseif m:lower():find("flash", 1, true) then
            table.insert(flash, m)
        end
    end
    table.sort(lite,  function(a, b) return a > b end)
    table.sort(flash, function(a, b) return a > b end)
    local out = {}
    for _, m in ipairs(lite)  do table.insert(out, m) end
    for _, m in ipairs(flash) do table.insert(out, m) end
    return out
end
...
-- Set the model for any provider and persist it.
function AIHelper:setProviderModel(provider, model_name)
    if not model_name or #model_name == 0 then return false end
    if not self.providers[provider] then return false end
    self.providers[provider].model = model_name
    self:saveModelToConfig(model_name, provider)
    return true
end

-- Dynamically fetch the model list for a provider from its /models endpoint,
-- filtering out models irrelevant to text analysis (image/audio/embedding/etc.).
-- Always sourced live from the provider; returns an empty list when no key /
-- offline / on error (the picker still offers manual model entry).
-- Results are cached; pass force=true to re-fetch.
function AIHelper:fetchModels(provider, force)
    local pcfg = self.providers[provider]
    if not pcfg then return {} end

    if not force and self._models_cache[provider] then
        return self._models_cache[provider]
    end

    local endpoint = (self.provider_models_endpoint or {})[provider]
    local key      = pcfg.api_key

    if not endpoint or not key or key == "" then
        return {}
    end
    ...
    local list = data.data or data.models or {}
    if type(list) ~= "table" then return {} end

    local keep = MODEL_KEEP[provider]
    local seen, filtered = {}, {}
    for _, m in ipairs(list) do
        local id = (type(m) == "table") and (m.id or m.name) or nil
        if type(id) == "string" and id ~= "" then
            id = id:gsub("^models/", "")              -- strip Gemini native prefix
            local active = (type(m) ~= "table") or (m.active == nil) or m.active
            if active and not seen[id] and not _modelBlocked(id)
               and (not keep or keep(id)) then
                seen[id] = true
    ...
    if provider == "gemini" then
        filtered = _sortGeminiFlash(filtered)
    else
        table.sort(filtered)
    end

    self._models_cache[provider] = filtered
    logger.info("AIHelper: Fetched", #filtered, "models for", provider)
    return filtered
end
...
function AIHelper:loadModelFromFile()
    ...
    -- Generic per-provider key/model loading for all UI providers
    -- (openai / claude / groq / gemini / deepseek).
    local xray_dir = DataStorage:getSettingsDir() .. "/xray/"
    for _, prov in ipairs(self.provider_order or {}) do
        if self.providers[prov] then
            local mf = io.open(xray_dir .. prov .. "_model.txt", "r")
            if mf then
                local m = mf:read("*a"):match("^%s*(.-)%s*$")
                mf:close()
                if m and #m > 0 then self.providers[prov].model = m end
            end
            local kf = io.open(xray_dir .. prov .. "_api_key.txt", "r")
            if kf then
                local k = kf:read("*a"):match("^%s*(.-)%s*$")
                kf:close()
                if k and #k > 0 then self.providers[prov].api_key = k end
            end
        end
    end
        -- Gemini API Key
...
-- =============================================================================
-- Raw AI calls for generator.lua (chunk_summary pipeline)
-- These return the raw text from the AI response without the old
-- validateAndCleanData schema processing.
-- =============================================================================

-- callAIRaw(prompt, system_prompt, config)
--   config: { type="gemini"|"chatgpt"|"local", api_key, model, endpoint }
-- Returns raw response text string, or nil + err_code, err_msg.
function AIHelper:callAIRaw(prompt, system_prompt, config)
    if not config then return nil, "no_config" end
    local ai_type = config.type or "gemini"
    if ai_type == "gemini" then
        return self:_callGeminiRaw(prompt, system_prompt, config)
    elseif ai_type == "claude" then
        return self:_callClaudeRaw(prompt, system_prompt, config)
    else
        return self:_callOpenAICompatibleRaw(prompt, system_prompt, config)
    end
end

-- Anthropic Claude (Messages API). Claude does not support response_format;
-- the chunk_summary prompt itself instructs the model to return pure JSON,
-- which generator.lua then strips of any markdown fences.
function AIHelper:_callClaudeRaw(prompt, system_prompt, config)
    if not self:checkNetworkConnectivity() then
        return nil, "error_no_network", "No internet connection"
    end

    local endpoint = config.endpoint or "https://api.anthropic.com/v1/messages"
    local model    = config.model or self.providers.claude.model or "claude-3-5-sonnet-20241022"
    local api_key  = config.api_key or self.providers.claude.api_key or ""
    ...
    if system_prompt and system_prompt ~= "" then
        req_tbl.system = system_prompt
    end
    local request_body = json.encode(req_tbl)

    for attempt = 1, self.MAX_RAW_RETRIES or 3 do
        if attempt > 1 then socket.sleep(4) end
        ...
        if code_num == 200 then
            ...
            if ok and data and data.content and data.content[1] and data.content[1].text then
                return data.content[1].text
            end
            return nil, "error_parse", "Unexpected Claude response: " .. response_text:sub(1, 200)
        elseif code_num == 429 then
            socket.sleep(8)
        elseif code_num == 503 or code_num == 504 then
            -- retry
        else
            return nil, "error_" .. tostring(code_num or "nil"),
                        "HTTP " .. tostring(code_num)
        end
    end
    return nil, "error_timeout", "Max retries exceeded"
end

function AIHelper:_callGeminiRaw(prompt, system_prompt, config)
    if not self:checkNetworkConnectivity() then
        return nil, "error_no_network", "No internet connection"
    end

    local model   = config.model or self.providers.gemini.model or "gemini-flash-lite-latest"
    local api_key = config.api_key or self.providers.gemini.api_key or ""
    ...
    if system_prompt and system_prompt ~= "" then
        req_tbl.systemInstruction = { parts = {{ text = system_prompt }} }
    end
    ...
    for attempt = 1, self.MAX_RAW_RETRIES or 3 do
        ...
        if code_num == 200 then
            ...
            if ok and data and data.candidates and data.candidates[1] then
                ...
                if cand.finishReason == "SAFETY" then
                    return nil, "error_safety", "Safety filter triggered"
                end
                if cand.content and cand.content.parts and cand.content.parts[1] then
                    return cand.content.parts[1].text
                end
            end
            return nil, "error_parse", "Unexpected Gemini response: " .. response_text:sub(1, 200)
        elseif code_num == 429 then
            socket.sleep(8)
        elseif code_num == 503 or code_num == 504 then
            -- retry
...
function AIHelper:_callOpenAICompatibleRaw(prompt, system_prompt, config)
    local endpoint = config.endpoint
                     or (self.providers[config.type or ""] and self.providers[config.type].endpoint)
                     or (config.type == "chatgpt"
                         and "https://api.openai.com/v1/chat/completions"
                         or self.providers["local"].endpoint)

    local is_local = endpoint and (
        endpoint:find("127%.0%.0%.1") or endpoint:find("localhost") or
        endpoint:find("192%.168%.")   or endpoint:find("^http://10%.")
    )
    if not is_local and not self:checkNetworkConnectivity() then
        return nil, "error_no_network", "No internet connection"
    end
    ...
    local request_func = (endpoint and endpoint:find("^http://"))
                         and http.request or https.request

    for attempt = 1, self.MAX_RAW_RETRIES or 3 do
        if attempt > 1 then socket.sleep(4) end

        local response_body = {}
        local _, code = request_func{
            url     = endpoint,
            method  = "POST",
            headers = {
                ["Content-Type"]   = "application/json",
        ...
        if code_num == 200 then
            ...
            if ok and data and data.choices and data.choices[1] then
                ...
                if msg and msg.content then return msg.content end
            end
            return nil, "error_parse", "Unexpected response: " .. response_text:sub(1, 200)
        elseif code_num == 429 then
            socket.sleep(8)
        elseif code_num == 503 or code_num == 504 then
            -- retry
